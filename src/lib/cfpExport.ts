...
export async function exportCFPToExcel(families: CFPFamily[], filterDescription: string) {
    ...
    // Define Columns
    worksheet.columns = [
        { header: 'Policy Group (Base)', key: 'base_policy', width: 20 },
        { header: 'Type', key: 'term_type', width: 14 },
        { header: 'Policy Number', key: 'policy_number', width: 22 },
        { header: 'Current Term?', key: 'is_current', width: 14 },
    ...
    for (const family of families) {
        for (const term of family.terms) {
            const row = worksheet.addRow({
                ...
                is_current: term.is_current ? 'Yes' : 'No',
            ...
            ['term_type', 'is_current', 'effective_date', 'expiration_date', 'has_dec', 'has_rce', 'has_dic', 'has_es', 'has_bamboo'].forEach(col => {
                const cell = row.getCell(col);
                cell.alignment = { vertical: 'middle', horizontal: 'center' };
            });

            // Soft highlight for Missing DEC/RCE
            if (!term.has_dec) {
